...
class Marshaller:
    ...
    def marshal(self, msg, tstamp):
        ...
        """#if sinetstream-upython
        bio = io.BytesIO()
        bio.write(avro_signle_object_format_marker)
        bio.write(message_schema_fingerprint)
        enc = uavro.io.BinaryEncoder(bio)
        enc.write_long(tstamp)
        enc.write_bytes(msg)
        return bio.getvalue()
        #endif
...
class Unmarshaller:
    ...
    def unmarshal(self, msg):
        ...
        """#if sinetstream-upython
        dec = uavro.io.BinaryDecoder(bio)
        #endif
        try:
            """#if sinetstream-python
            m = self._reader.read(decoder)
            """#if sinetstream-upython
            tstamp = dec.read_long()
            bs = dec.read_bytes()
            return tstamp, bs
            #endif
