...
from sinetstream.utils import Epoch
from sinetstream.value_type import TEXT, TextValueType, ByteArrayValueType
#endif
from sinetstream.compression import compression_registry
from sinetstream.packet import Packet

#if sinetstream-upython
broker_type_reader_registry = Registry("sinetstream.reader", PluginMessageReader)
broker_type_writer_registry = Registry("sinetstream.writer", PluginMessageWriter)
#endif
...
"""#if sinetstream-upython
class Consistency:
    AT_MOST_ONCE = (1, "AT_MOST_ONCE")
    AT_LEAST_ONCE = (2, "AT_LEAST_ONCE")
    EXACTLY_ONCE = (3, "EXACTLY_ONCE")

    i2c = { 0: AT_MOST_ONCE,
            1: AT_LEAST_ONCE,
            2: EXACTLY_ONCE }
    s2c = { "AT_MOST_ONCE": AT_MOST_ONCE,
    ...
    def get(x):
        if isinstance(x, tuple):
            return x
        if isinstance(x, int):
            return Consistency.i2c[x]
        elif isinstance(x, str):
            return Consistency.s2c[x]
#endif
...
def convert_params(params):
    ...
    try:
        ...
        new_params["consistency"] = (
            consistency if isinstance(consistency, Consistency)
            else Consistency.get(consistency))
        #endif
        return new_params
...
class Message:
    ...
    def timestamp(self):
        ...
        """#if sinetstream-upython
        if self._tstamp == Message.NOTSTAMP:
            return Message.NOTSTAMP
        else:
            return float(self._tstamp) / 1000000
...
def make_client_id():
    ...
    """#if sinetstream-upython
    import random
    return f"sinetstream-{random.randint(10000000,99999999)}"
    #endif
...
class BaseMessageReader(MessageIO):
    ...
    default_params["receive_timeout_ms"] = float("inf")
    #endif
    ...
    def _make_message(self, value, topic, raw):
        ...
        """#if sinetstream-upython
        value_tstamp = value.timestamp if hasattr(value, "timestamp") else Message.NOTSTAMP
        #endif
        mlen = len(value)
        ...
        #if sinetstream-upython
        if tstamp != Message.NOTSTAMP:
            tstamp = self.epoch.posix2sys(tstamp)
        #endif
    ...
    def _setup_deserializer(self):
        ...
        """#if sinetstream-upython
        value_type_class = { TEXT: TextValueType,
                             BYTE_ARRAY: ByteArrayValueType }.get(value_type)
        #endif
...
class MessageReader(BaseMessageReader):
    ...
    """#if sinetstream-upython
    registry = broker_type_reader_registry
    #endif
...
class BaseMessageWriter(MessageIO):
    ...
    def __init__(self,
        ...
        self.message_format = params.get("message_format", 3)
        #if sinetstream-upython
        self.use_realtime_clock = params.get("use_realtime_clock", True)
        self.epoch = Epoch()
        #endif
    ...
    def _publish(self, msg, timestamp):
        ...
        if timestamp is not None:
            tstamp = int(timestamp * 1000_000)
        else:
            if self.use_realtime_clock:
                tstamp = time.time_ns() // 1000
            else:
                tstamp = Message.NOTSTAMP
        if tstamp != Message.NOTSTAMP:
            tstamp = self.epoch.sys2posix(tstamp)
        #endif
...
class MessageWriter(BaseMessageWriter):
    ...
    """#if sinetstream-upython
    registry = broker_type_writer_registry
    #endif
...
#if sinetstream-upython
class MqttReaderEntryPoint:
    @classmethod
    def load(cls):
        from sinetstreamplugin.mqtt import MqttReader
        return MqttReader


class MqttWriterEntryPoint:
    @classmethod
    def load(cls):
        from sinetstreamplugin.mqtt import MqttWriter
        return MqttWriter

broker_type_reader_registry.register("mqtt", MqttReaderEntryPoint)
broker_type_writer_registry.register("mqtt", MqttWriterEntryPoint)

class IOStreamWriterEntryPoint:
    @classmethod
    def load(cls):
        from sinetstreamplugin.iostream import IOStreamWriter
        return IOStreamWriter

class IOStreamReaderEntryPoint:
    @classmethod
    def load(cls):
        from sinetstreamplugin.iostream import IOStreamReader
        return IOStreamReader

broker_type_reader_registry.register("iostream", IOStreamReaderEntryPoint)
broker_type_writer_registry.register("iostream", IOStreamWriterEntryPoint)
#endif
